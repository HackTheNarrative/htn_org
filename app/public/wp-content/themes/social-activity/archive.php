...
if (!have_posts()) : 
	echo '<h2>' . esc_html__('No posts found', 'social-activity') . '</h2>';
else : 
		...
		if (get_post_type() == 'post') {
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('post cmsms_default_type'); ?>>
				<div class="cmsms_post_info entry-meta">
					<span class="cmsms_post_format_img <?php echo (get_post_type() == 'profile' ? 'cmsms-icon-user-2' : 'cmsms-icon-pencil-2'); ?>"></span>
					<?php 
				...
				<?php
					...
					'<div class="cmsms_post_cont_info entry-meta">' . 
						'<span class="cmsms_post_user_name">' . 
							...
							'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" rel="author" title="' . esc_attr__('Posts by', 'social-activity') . ' ' . esc_attr(get_the_author_meta('display_name')) . '">' . get_the_author_meta('display_name') . '</a>' . 
						'</span>';
					...
					if (get_post_type() == 'project' && get_the_terms(get_the_ID(), 'pj-tags')) {
						echo '<span class="cmsms_post_tags">' . 
							esc_html__('Tags', 'social-activity') . ' ' . 
					...
					'<footer class="cmsms_post_footer entry-meta">' . 
						'<div class="cmsms_post_meta_info">' . 
							...
							(comments_open() ? '<a class="cmsms_post_comments cmsms-icon-comment-1" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'social-activity') . ' ' . esc_attr(get_the_title()) . '">' . get_comments_number() . '</a>' : '') . 
						'</div>' . 
						'<a class="button cmsms_post_read_more" href="' . esc_url(get_permalink(get_the_ID())) . '">' . esc_html__('Read More', 'social-activity') . '</a>' . 
					'</footer>';
				?>
				</div>
			</article>
