...
function cmsms_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = '<abbr class="published cmsms_slider_post_date cmsms-icon-calendar-2" title="' . esc_attr(get_the_date()) . '">' . 
			get_the_date() . 
		'</abbr>' . 
		...
		$out = '<abbr class="published cmsms_slider_project_date cmsms-icon-calendar-2" title="' . esc_attr(get_the_date()) . '">' . 
			get_the_date() . 
		'</abbr>' . 
		'<abbr class="dn date updated" title="' . esc_attr(get_the_modified_date()) . '">' . 
			get_the_modified_date() . 
...
function cmsms_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = '<span class="cmsms_slider_post_user_name">' . 
			...
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'social-activity') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . get_the_author_meta('display_name') . '</span></a>' . 
		'</span>';
	} elseif ($type == 'project') {
...
function cmsms_slider_post_comments($type = 'post', $show = true) {
	if (comments_open()) {
		if ($type == 'post') {
			$out = '<a class="cmsms_slider_post_comments cmsms-icon-comment-1" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'social-activity') . ' ' . esc_attr(get_the_title()) . '">' . get_comments_number() . '</a>';
		} elseif ($type == 'project') {
			$out = '<a class="cmsms_slider_project_comments cmsms-icon-comment-1" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'social-activity') . ' ' . esc_attr(get_the_title()) . '">' . get_comments_number() . '</a>';
		}
