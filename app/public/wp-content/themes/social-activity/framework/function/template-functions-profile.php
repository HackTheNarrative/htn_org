		...
		if ($template_type == 'page') {
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_profile_post_cat']) {
				$out = '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'social-activity') . ':' . '</div>' . 
...
function cmsms_profile_comments($template_type = 'page', $show = true) {
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = '<a class="cmsms_profile_comments cmsms-icon-comment-1" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'social-activity') . ' ' . esc_attr(get_the_title()) . '">' . get_comments_number() . '</a>';
		} elseif ($template_type == 'post') {
			$cmsms_option = cmsms_get_global_options();
			$out = '';
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_profile_post_comment']) {
				$out = '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Comments', 'social-activity') . ':' . '</div>' . 
					'<div class="profile_details_item_desc">' . 
						'<a class="cmsms_profile_comments cmsms-icon-comment-1" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'social-activity') . ' ' . esc_attr(get_the_title()) . '">' . get_comments_number() . '</a>' . 
					'</div>' . 
				'</div>';
			}
		}
