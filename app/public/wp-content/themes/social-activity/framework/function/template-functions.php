...
function cmsms_logo() {
	...
	if ($cmsms_option[CMSMS_SHORTNAME . '_logo_type'] == 'text') {
		...
		if ($cmsms_option[CMSMS_SHORTNAME . '_logo_subtitle'] != '') {
			...
			$blog_descr = (get_bloginfo('description')) ? get_bloginfo('description') : esc_html__('Default Logo Subtitle', 'social-activity');
		}
						...
						if (is_search()) {
							...
							if (!empty($wp_query->found_posts)) {
								if ($wp_query->found_posts > 1) {
									echo $wp_query->found_posts . ' ' . esc_html__('search results for', 'social-activity') . ': ' . esc_attr(get_search_query());
								} else {
									echo $wp_query->found_posts . ' ' . esc_html__('search result for', 'social-activity') . ': ' . esc_attr(get_search_query());
								}
							} else {
								echo esc_html__('Search results for', 'social-activity') . ': ' . esc_attr(get_search_query());
							}
						} elseif (is_archive()) {
							if (is_day()) {
								echo esc_html__('Archive for date', 'social-activity') . ': ' . get_the_date();
							} elseif (is_month()) {
								echo esc_html__('Archive for month', 'social-activity') . ': ' . get_the_date('F Y');
							} elseif (is_year()) {
								echo esc_html__('Archive for year', 'social-activity') . ': ' . get_the_date('Y');
							} elseif (is_category()) {
								echo single_cat_title(esc_html__('Archive for category', 'social-activity') . ': ', false);
							} elseif (is_tag()) {
								echo single_tag_title(esc_html__('Archive for tag', 'social-activity') . ': ', false);
							} elseif (is_author()) {
								$archive_author = (get_query_var('author_name')) ? get_user_by('slug', get_query_var('author_name')) : get_userdata(get_query_var('author'));
								
								esc_html_e('Author archive', 'social-activity');
								
								if (isset($archive_author->nickname)) {
									echo ' ' . esc_html__('for', 'social-activity') . ': ' . $archive_author->nickname;
								}
							} elseif (is_tax()) {
								$archive_taxonomy = get_taxonomy(get_query_var('taxonomy'));
								
								esc_html_e('Archive for', 'social-activity');
								...
								echo post_type_archive_title('', false) . ' ' . esc_html__('archive', 'social-activity');
							} else {
								esc_html_e('Archives', 'social-activity');
							}
						}
...
function cmsms_thumb_rollover($cmsms_id, $type = 'post-thumbnail', $rollover = true, $open_link = true, $group = false, $attachment_images = false, $attachment_video_type = false, $attachment_video_link = false, $attachment_video_links = false, $highImg = false, $show = true, $link_redirect = false, $link_url = false) {
	...
	if (has_post_thumbnail($cmsms_id)) {
		...
		$out .= '<span class="img_placeholder cmsms-icon-camera-2"></span>';
	...
	if ($rollover) {
		if (
			...
			$out .= '<a ' . ($is_video_selfhosted ? $shv_out : 'href="' . ((!$attachment_video_link) ? $cmsms_image_link[0] : embedConvert($attachment_video_link)) . '"') . ' rel="ilightbox[' . $cmsms_id . '_' . $unique_id . ']" title="' . esc_attr($cmsms_title) . '" class="cmsms_image_link' . (($open_link) ? '' : ' no_open_link') . '"><span class="cmsms-icon-search-2"></span></a>';
		}
		
		
		if ($open_link) {
			$out .= '<a href="' . (($link_redirect == 'true' && $link_url != '') ? esc_url($link_url) : esc_url(get_permalink($cmsms_id))) . '" title="' . esc_attr($cmsms_title) . '" class="cmsms_open_link"><span class="cmsms-icon-attach-2"></span></a>';
		}
	} else {
	...
	$out = '<figure class="alignleft">' . 
		...
		if (has_post_thumbnail()) {
			...
			if (get_post_format() == 'aside') {
				$out .= '<span class="img_placeholder cmsms-icon-megaphone-1"></span>';
			} elseif (get_post_format() == 'audio') {
				$out .= '<span class="img_placeholder cmsms-icon-music"></span>';
			} elseif (get_post_format() == 'chat') {
				$out .= '<span class="img_placeholder cmsms-icon-star-2"></span>';
				...
				$out .= '<span class="img_placeholder cmsms-icon-globe-1"></span>';
			} elseif (get_post_format() == 'quote') {
				$out .= '<span class="img_placeholder cmsms-icon-comment-1"></span>';
			} elseif (get_post_format() == 'status') {
				$out .= '<span class="img_placeholder cmsms-icon-user-2"></span>';
			} elseif (get_post_format() == 'video') {
				$out .= '<span class="img_placeholder cmsms-icon-videocam-1"></span>';
			} else {
				$out .= '<span class="img_placeholder cmsms-icon-monitor"></span>';
			}
...
?>	
		<div class="fl share_posts_item">
			<a href="https://twitter.com/share" class="twitter-share-button" data-lang="en"><?php esc_html_e('Tweet', 'social-activity'); ?></a>
		...
		<div class="fl share_posts_item">
			<a href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode(get_permalink(get_the_ID())); ?>" class="pin-it-button" count-layout="horizontal">
				<img border="0" src="//assets.pinterest.com/images/PinExt.png" title="<?php _e('Pin It', 'social-activity'); ?>" />
			</a>
			<script type="text/javascript">
				(function (d, s, id) { 
					var js = undefined, 
...
function cmsms_author_box($title_box = false, $tag = 'h3') {
	...
	if ($user_description) {
		...
		if ($user_email) {
			echo '<li>' . 
				'<a href="mailto:' . $user_email . '" class="cmsms-icon-mail-2" title="' . esc_attr__('Email', 'social-activity') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_url) {
			echo '<li>' . 
				'<a href="' . esc_url($user_url) . '" class="cmsms-icon-globe-1" title="' . esc_attr__('Website', 'social-activity') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_googleplus) {
			echo '<li>' . 
				'<a href="https://plus.google.com/+' . $user_googleplus . '" class="cmsms-icon-gplus-1" title="' . esc_attr__('Website', 'social-activity') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_twitter) {
			echo '<li>' . 
				'<a href="https://twitter.com/' . $user_twitter . '" class="cmsms-icon-twitter-1" title="' . esc_attr__('Twitter', 'social-activity') . '" target="_blank"></a>' . 
			'</li>';
		}
		
		if ($user_facebook) {
			echo '<li>' . 
				'<a href="' . esc_url($user_facebook) . '" class="cmsms-icon-facebook-1" title="' . esc_attr__('Facebook', 'social-activity') . '" target="_blank"></a>' . 
			'</li>';
		}
...
function cmsms_related($tag = 'h3', $related_box = false, $tgsarray = null, $popular_box = false, $recent_box = false, $related_number = 4, $type = 'post', $taxonomy = null) {
	if (($related_box && !empty($tgsarray)) || $recent_box || $popular_box) {
		...
		if ($type == 'post') {
			echo esc_html__('More posts', 'social-activity');
		} else {
			echo esc_html__('More projects', 'social-activity');
		}
		...
		if ($related_box && !empty($tgsarray) && $r->have_posts()) {
			echo '<li>' . 
				'<a href="#" class="current">' . esc_html__('Related', 'social-activity') . '</a>' . 
			'</li>';
		}
		...
		if ($popular_box) {
			...
			echo '">' . esc_html__('Popular', 'social-activity') . '</a>' . 
			'</li>';
		}
		...
		if ($recent_box) {
			...
			echo '">' . esc_html__('Latest', 'social-activity') . '</a>' . 
			'</li>';
		}
		...
		if ($related_box && !empty($tgsarray) && $r->have_posts()) {
			...
			if ($related->have_posts()) :
				...
				echo '<h5>';
				if ($type == 'post') {
					echo esc_html__('No related posts found', 'social-activity');
				} else {
					echo esc_html__('No related projects found', 'social-activity');
				}
				echo '</h5>';
			endif;
		...
		if ($popular_box) {
			...
			if ($popular->have_posts()) :
				...
				echo '<h5>';
				if ($type == 'post') {
					echo esc_html__('No popular posts found', 'social-activity');
				} else {
					echo esc_html__('No popular projects found', 'social-activity');
				}
				echo '</h5>';
			endif;
		...
		if ($recent_box) { 
			...
			if ($recent->have_posts()) :
				...
				echo '<h5>';
				if ($type == 'post') {
					echo esc_html__('No latest posts found', 'social-activity');
				} else {
					echo esc_html__('No latest projects found', 'social-activity');
				}
				echo '</h5>';
			endif;
