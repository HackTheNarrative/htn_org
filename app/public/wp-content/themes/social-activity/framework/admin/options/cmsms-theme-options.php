...
function cmsms_admin_enqueue_scripts($hook) {
	if ( 
		...
		wp_localize_script('cmsms_theme_options_js', 'cmsms_options', array( 
			'create_gallery' => 	esc_attr__('Create Gallery', 'social-activity'), 
			'select_format' => 		esc_attr__('Please select the format.', 'social-activity'), 
			'link_exists' => 		esc_html__('Link with this format already exists.', 'social-activity'), 
			'want_remove' => 		esc_html__('Do you realy want to remove this item?', 'social-activity'), 
...
function show_cmsms_meta_box() {
	...
	foreach ($custom_all_meta_fields as $field) {
		...
		case 'select_sidebar':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Default Sidebar', 'social-activity') . '</option>';
		...
		case 'select_slider':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Slider', 'social-activity') . '</option>';
		...
		case 'select_post_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Blog Category', 'social-activity') . '</option>';
		...
		case 'select_project_cat':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Project Category', 'social-activity') . '</option>';
		...
		case 'select_pl_categ':
			...
			echo '<select name="' . $field['id'] . '" id="' . $field['id'] . '">' . 
				'<option value="">' . esc_html__('Select Profile Category', 'social-activity') . '</option>';
		...
		case 'rgba':
			...
			'<div class="cl"></div>' . 
			'<input type="range" name="' . $field['id'] . '[1]" id="' . $field['id'] . '_1" value="' . $meta[1] . '" size="30" min="0" max="100" step="1" />' . 
			'<input type="text" name="' . $field['id'] . '_number" id="' . $field['id'] . '_number" value="' . $meta[1] . '" size="5" readonly="readonly" />' . 
			'<span class="description">' . $field['desc'] . '</span>' . 
			'<script type="text/javascript"> ' . 
				'jQuery(document).ready(function () { ' . 
					'(function ($) { ' . 
		...
		case 'icon':
			echo '<div class="icon_management">' . 
				'<p>' . 
					...
					'<input id="' . $field['id'] . '_button" class="cmsms_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'social-activity') . '" />' . 
					'<a href="#" class="cmsms_remove_icon admin-icon-remove" title="' . esc_attr__('Remove icon', 'social-activity') . '"' . (($meta != '') ? ' style="display:inline-block;"' : '') . '></a>' . 
				'</p>' . 
			'</div>' . 
		...
		case 'repeatable_link':
			...
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'social-activity') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Link', 'social-activity') . '</strong></div>' . 
			'</div>' . 
			...
			'<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<optgroup label="' . __('Blank Field', 'social-activity') . '">' . 
					'<option value="">' . esc_html__('Select Link', 'social-activity') . '</option>' . 
				'</optgroup>' . 
				'<optgroup label="' . __('Posts', 'social-activity') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . __('Pages', 'social-activity') . '">';
			...
			echo '</optgroup>' . 
				'<optgroup label="' . __('Projects', 'social-activity') . '">';
		...
		case 'repeatable_multiple':
			echo '<div class="ovh">' . 
				'<div class="fl"><strong>' . esc_html__('Title', 'social-activity') . '</strong></div>' . 
				'<div class="fl"><strong>' . esc_html__('Values', 'social-activity') . '</strong></div>' . 
			'</div>' . 
			'<ul id="' . $field['id'] . '-repeatable" class="custom_repeatable">';
		...
		case 'repeatable_media':
			echo '<select name="' . $field['id'] . '-select" id="' . $field['id'] . '-select">' . 
				'<option value="">' . esc_html__('Select Format', 'social-activity') . ' &nbsp;</option>';
				...
				foreach ($ids as $id) {
					if (isset($id[0]) && isset($id[1])) {
						echo '<li class="cmsms_gallery_item">' . 
							...
							'<a href="#" class="cmsms_gallery_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'social-activity') . '"></a>' . 
						'</li>';
					}
				}
			}
...
function add_custom_cmsms_meta_box() {
    add_meta_box( 
		...
		__('Theme Options', 'social-activity'), 
		'show_cmsms_meta_box', 
		'', 
		'normal', 
		'high' 
