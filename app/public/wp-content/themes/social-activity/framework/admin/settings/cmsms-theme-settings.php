...
function cmsms_get_settings() {
	...
	case CMSMS_PAGE_BASENAME:
		...
		$cmsms_settings_page_title = esc_html__('Theme General Settings', 'social-activity');	
	...
	case CMSMS_PAGE_BASENAME . '-style':
		...
		$cmsms_settings_page_title = esc_html__('Theme Appearance', 'social-activity');
	...
	case CMSMS_PAGE_BASENAME . '-font':
		...
		$cmsms_settings_page_title = esc_html__('Theme Fonts Settings', 'social-activity');
	...
	case CMSMS_PAGE_BASENAME . '-color':
		...
		$cmsms_settings_page_title = esc_html__('Theme Color Schemes', 'social-activity');
	...
	case CMSMS_PAGE_BASENAME . '-single':
		...
		$cmsms_settings_page_title = esc_html__('Theme Single Posts Settings', 'social-activity');
	...
	case CMSMS_PAGE_BASENAME . '-demo':
		...
		$cmsms_settings_page_title = esc_html__('Theme Settings Import/Export', 'social-activity');
...
function cmsms_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		__('Default Field', 'social-activity'), 
		'desc' => 		__('This is a default description.', 'social-activity'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function cmsms_settings_scripts() {
	...
	wp_localize_script('cmsms_theme_settings_js', 'cmsms_setting', array( 
		...
		'remove_sidebar' => 	esc_html__('Do you realy want to remove this sidebar?', 'social-activity'), 
		'find' => 				esc_attr__('Find icons', 'social-activity'), 
		'remove_icon' => 		esc_html__('Do you realy want to remove this social icon?', 'social-activity'), 
		'theme_uri' => 			get_template_directory_uri(), 
		'shortname' => 			CMSMS_SHORTNAME, 
		'done' => 				esc_html__('All theme settings were imported successfully.', 'social-activity'), 
		'fail' => 				esc_html__("Error on page!!!\nPlease reload page and try again.", 'social-activity'), 
...
function cmsms_add_menu() {
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'social-activity'), 
		esc_attr__('Theme Settings', 'social-activity'), 
		'manage_options', 
		CMSMS_PAGE_BASENAME, 
	...
	$cmsms_settings_general = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme General Settings', 'social-activity'), 
		esc_attr__('General', 'social-activity'), 
		'manage_options', 
		CMSMS_PAGE_BASENAME, 
		'cmsms_settings_page_fn' 
	);
	...
	$cmsms_settings_style = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Appearance', 'social-activity'), 
		esc_attr__('Appearance', 'social-activity'), 
		'manage_options', 
	...
	$cmsms_settings_font = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Fonts Settings', 'social-activity'), 
		esc_attr__('Fonts', 'social-activity'), 
		'manage_options', 
	...
	$cmsms_settings_color = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Color Schemes', 'social-activity'), 
		esc_attr__('Colors', 'social-activity'), 
		'manage_options', 
	...
	$cmsms_settings_single = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Single Posts Settings', 'social-activity'), 
		esc_attr__('Single Posts', 'social-activity'), 
		'manage_options', 
	...
	$cmsms_settings_demo = $add_submenu_p( 
		CMSMS_PAGE_BASENAME, 
		esc_attr__('Theme Settings Import/Export', 'social-activity'), 
		esc_attr__('Import/Export', 'social-activity'), 
		'manage_options', 
...
function cmsms_settings_page_fn() {
	...
	if ($current_tab != 'wpml') {
		...
		'<p class="submit-top">' . 
			(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'social-activity') . '" />' : '') . 
		'</p>';
		...
		echo '<p class="submit">' . 
				(($current_tab != 'import' && $current_tab != 'export') ? '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'social-activity') . '" />' : (($current_tab == 'import') ? '<input name="import" type="button" class="button button-primary button-large cmsms-demo-import" value="' . esc_attr__('Import Settings', 'social-activity') . '" />' : '')) . 
			'</p>' . 
		'</form>';
	} else {
		do_settings_sections(__FILE__);
...
function cmsms_form_field_fn($args = array()) {
	...
	case 'rgba':
		...
		'<br />' . 
		'<small>' . esc_html__('Color opacity', 'social-activity') . ':</small>' . 
		'<div class="cl"></div>' . 
	...
	case 'opacity':
		...
		echo '<input type="range" name="' . $cmsms_option_name . '[' . $id . ']" id="' . $id . '" value="' . $options[$id] . '" size="30" min="0" max="100" step="1" class="cmsms-range-field ' . $field_class . '" />' . 
		'<input type="text" name="' . $cmsms_option_name . '_number" id="' . $cmsms_option_name . '_number" value="' . $options[$id] . '" size="5" class="cmsms-range-field-number" readonly="readonly" />' . 
		'<span class="description">' . 
			(($desc != '') ? $desc . '<br />' : '') . 
		'</span>';
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'social-activity') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'social-activity') . '</label>' . 
			'</div>';
		}
		...
		if ($font_color) {
			echo '<div class="cmsms_admin_block" style="padding-bottom:20px;">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_color" style="padding-bottom:15px;">' . esc_html__('Font Color', 'social-activity') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsms_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'social-activity') . ' <em>(' . __('in pixels', 'social-activity') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsms_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'social-activity') . ' <em>(' . __('in pixels', 'social-activity') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'social-activity') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'social-activity') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'social-activity') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsms_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'social-activity') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'social-activity') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsms_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'social-activity') . '" />' . 
				'<a href="#" class="cmsms_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'social-activity') . '"></a>' . 
			'</p>' . 
			...
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_link">' . 
					...
					__('Icon Link', 'social-activity') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					__('Icon Title', 'social-activity') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					__('Open link in a new tab/window?', 'social-activity') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'social-activity') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'social-activity') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'social-activity') . '"></a> ' . 
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : __('Expecting a Numeric value!', 'social-activity');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						CMSMS_SHORTNAME . '_txt_numeric_error',
						__('Expecting a Numeric value! Please fix.', 'social-activity'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values', 'social-activity');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						...
						__('Expecting comma separated numeric values! Please fix.','social-activity'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : __('Invalid email! Please re-enter!', 'social-activity');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.', 'social-activity');
				}
				...
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						...
						__('Please enter a valid email address.', 'social-activity'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : __('Number!', 'social-activity');
...
function theme_admin_bar_render() {
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Theme Settings', 'social-activity'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('General', 'social-activity'), 
		'href' => admin_url('admin.php?page=cmsms-settings') 
	));
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Appearance', 'social-activity'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Fonts', 'social-activity'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Colors', 'social-activity'), 
	...
	$wp_admin_bar->add_menu(array( 
		...
		'title' => esc_attr__('Single', 'social-activity'), 
