					...
					echo '<div class="' . $cmsms_project_img_columns . '">' . 
						'<figure class="cmsms_img_rollover_wrap preloader">' . 
							...
							'<div class="cmsms_img_rollover">' . 
								'<a href="' . esc_url($link_href[0]) . '" rel="ilightbox[' . get_the_ID() . '_' . $unique_id . ']" title="' . cmsms_title(get_the_ID(), false) . '" class="cmsms_image_link no_open_link"><span class="cmsms-icon-search-2"></span></a>' . 
							'</div>' . 
						'</figure>' . 
					'</div>';
	...
	if ($project_sidebar == 'true') {
		echo '<div class="project_sidebar">';
			...
			if ($cmsms_project_sharing_box == 'true') {
				cmsms_sharing_box(esc_html__('Like this project?', 'social-activity'), 'h2');
			}
