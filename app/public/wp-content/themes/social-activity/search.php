		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('cmsms_search_post'); ?>>
			...
			<div class="cmsms_search_post_cont">
				...
				<footer class="cmsms_search_post_footer entry-meta">
					...
					<?php 
					echo '<abbr class="published cmsms_search_post_date cmsms-icon-calendar-2" title="' . esc_attr(get_the_date()) . '">' . 
						get_the_date() . 
					'</abbr>';
					
					
					if (comments_open()) {
						echo '<a class="cmsms_search_post_comments cmsms-icon-comment-1" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'social-activity') . ' ' . esc_attr(get_the_title()) . '">' . get_comments_number() . '</a>';
					}
					?>
					</div>
					<?php 
					echo '<a class="button cmsms_search_post_read_more" href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'social-activity') . '</a>'
					?>
				</footer>
			</div>
		</article>
