...
<div class="entry">
	<div class="error">
		<div class="error_bg" style="background:rgba(<?php 
			...
			?>">
				<h1 class="error_title"><?php echo esc_html__('404', 'social-activity'); ?></h1>
			</div>
		</div>
	</div>
</div>
<section class="content_wrap fullwidth">
	<div class="error">
		<?php 
			echo '<h2 class="error_subtitle">' . esc_html__("We're sorry, but the page you were looking for doesn't exist.", 'social-activity') . '</h2>';
			...
			if ($cmsms_option[CMSMS_SHORTNAME . '_error_sitemap_button'] && $cmsms_option[CMSMS_SHORTNAME . '_error_sitemap_link'] != '') {
				echo '<div class="error_button_wrap"><a href="' . esc_url($cmsms_option[CMSMS_SHORTNAME . '_error_sitemap_link']) . '" class="cmsms_button">' . esc_html__('Sitemap', 'social-activity') . '</a></div>';
			}
		?>
	</div>
